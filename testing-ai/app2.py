...
def extract_bank_statement_data(text: str) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    # Enhanced bank statement header patterns with more variations
    header_patterns = {
        "account_holder": [
            r"Account.*?Holder.*?[:]\s*([^\n\r]+)",
            r"Customer.*?Name.*?[:]\s*([^\n\r]+)",
        ...
        "opening_balance": [
            ...
            r"Balance.*?Forward.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Beginning.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Starting.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Previous.*?Statement.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            # More flexible patterns
            r"(?:Opening|Previous|Beginning|Starting).*?(?:Balance|Bal).*?\$?([\d,]+\.?\d*)",
            r"Balance.*?brought.*?forward.*?\$?([\d,]+\.?\d*)",
            r"(?:^|\n)\s*(?:Opening|Previous|Beginning)\s+.*?\$?([\d,]+\.?\d*)",
            # Pattern for when balance appears after date range
            r"(?:from|From)\s+\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}.*?(?:to|To)\s+\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}.*?\$?([\d,]+\.?\d*)"
        ],
        "closing_balance": [
            r"Closing.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Final.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Statement.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Current.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"Ending.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            r"New.*?Balance.*?[:]\s*\$?([\d,]+\.?\d*)",
            # More flexible patterns
            r"(?:Closing|Final|Current|Ending|New).*?(?:Balance|Bal).*?\$?([\d,]+\.?\d*)",
            r"Balance.*?as.*?of.*?\$?([\d,]+\.?\d*)",
            r"(?:^|\n)\s*(?:Closing|Final|Current|Ending)\s+.*?\$?([\d,]+\.?\d*)",
            # Pattern for balance at end of statement
            r"(?:balance|Balance).*?(?:end|End).*?\$?([\d,]+\.?\d*)"
        ],
        "bank_name": [
            r"([A-Z][a-z]+\s+Bank)",
            r"([A-Z]+\s+BANK)",
            r"Bank.*?[:]\s*([^\n\r]+)",
            r"([A-Z][a-z]+\s+(?:Bank|Credit Union|Financial))",
            r"^([A-Z][A-Za-z\s]+(?:Bank|BANK))"
        ]
    }
    
    # Extract statement header information with enhanced logic
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if field_name in ["opening_balance", "closing_balance"]:
                    ...
                    try:
                        balance_value = float(clean_value)
                        # Only accept reasonable balance values (not too large, not negative unless explicitly marked)
                        if abs(balance_value) < 999999999:  # Reasonable upper limit
                            statement_info[field_name] = balance_value
                            break
                    except ValueError:
                        continue
                else:
                    # For non-balance fields, just clean and store
                    if len(value) > 1 and not value.isdigit() or field_name == "account_number":
    ...
    # Additional balance extraction using table-like patterns
    if statement_info.get("opening_balance", 0) == 0 or statement_info.get("closing_balance", 0) == 0:
        # Look for balance information in summary tables
        balance_table_patterns = [
            # Look for summary table with previous and current balance
            r"Previous.*?Balance.*?\$?([\d,]+\.?\d*).*?Current.*?Balance.*?\$?([\d,]+\.?\d*)",
            r"Balance.*?brought.*?forward.*?\$?([\d,]+\.?\d*).*?Balance.*?carried.*?forward.*?\$?([\d,]+\.?\d*)",
            # Look for beginning and ending in same line or nearby lines
            r"Beginning.*?\$?([\d,]+\.?\d*).*?Ending.*?\$?([\d,]+\.?\d*)",
            r"(?:^|\n).*?(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}).*?\$?([\d,]+\.?\d*).*?(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}).*?\$?([\d,]+\.?\d*)"
        ]
        
        for pattern in balance_table_patterns:
            match = re.search(pattern, text, re.IGNORECASE | re.MULTILINE | re.DOTALL)
            if match:
                try:
                    if len(match.groups()) >= 2:
                        opening_val = match.group(1).replace(",", "").replace("$", "")
                        closing_val = match.group(2).replace(",", "").replace("$", "")
                        
                        if statement_info.get("opening_balance", 0) == 0:
                            statement_info["opening_balance"] = float(opening_val)
                        if statement_info.get("closing_balance", 0) == 0:
                            statement_info["closing_balance"] = float(closing_val)
                        break
